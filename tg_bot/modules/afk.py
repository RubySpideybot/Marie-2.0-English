...
def no_longer_afk(bot: Bot, update: Update):
    ...
    if res:
        update.effective_message.reply_text("{} came back to the keyboard now!".format(update.effective_user.first_name))
    ...
    if message.entities and message.parse_entities([MessageEntity.TEXT_MENTION, MessageEntity.MENTION]):
        ...
        for ent in entities:
            ...
            if sql.is_afk(user_id):
                ...
                if not user.reason:
                    res = "{} is away from the keyboard!\n\n<b>Reason</b> : {} ".format(fst_name)
                else:
                    res = "{} is away from the keyboard!\n\n<b>Reason</b> : {}. ".format(fst_name, user.reason)
